...
<head>
    ...
    <script type="text/javascript">
                ...
                columns: [[
                    ...
                    { field: 'function', title: '操作', width: 50, align: "center",
                        formatter: function(value,row,index) {
                            return "<a href='javascript:DeleteById(" + row.id +
                                ")'> 删除</a>|<a href='javascript:selectStreetByDistrict("+row.id+")'> 查看区域街道</a>";
                        }
                    }
                ]]
        ...
        //打开区域里面的街道
        function selectStreetByDistrict(id) {
            $("#DistrictStreet").dialog("open").dialog('setTitle',"街道");
            $('#street').datagrid({
                url:'/admin/getStreet?id='+id,
                width: 540,
                height: 400,
                fitColumns: true,
                pagination: true,
                ...
                columns: [[
                    {field:'ck',checkbox:true},  //复选框列
                    { field: 'id', title: '编号', width: 50, align: "center" },
                    { field: 'name', title: '路名', width: 50, align: "center" },
                    { field: 'function', title: '操作', width: 50, align: "center",
                        formatter: function(value,row,index){
     ...
     //修改提交按钮
       function SaveDialog2() {
           $('#upDistrictForm').form('submit',{
               url:"/admin/updateDistrict",//提交服务器地址
               success:function (data) {//成功的回调函数
                   //data接收服务器返回的json字符串（不是json对象）
                   var obj=$.parseJSON(data);
                   if (obj.result>0) {
...
<%--查看区域内所有的街道--%>
<div id="DistrictStreet" class="easyui-dialog" buttons="#districtStreetButtons"
     style="width: 600px; height: 500px; padding: 10px 20px;" closed="true">
    <table id="street"></table>
</div>
<div id="districtStreetButtons">
    <a href="javascript:SaveDialog2()" class="easyui-linkbutton"
       iconCls="icon-ok">修改</a>
    <a href="javascript:CloseDialog('DistrictStreet')"
       class="easyui-linkbutton" iconCls="icon-cancel">取消</a>
</div>
</body>
</html>
